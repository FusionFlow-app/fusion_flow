defmodule FusionFlowWeb.FlowAiCreatorLive do
  ...
  def handle_event("send_message", %{"content" => content}, socket) do
    if content == "" do
      ...
      socket = assign(socket, messages: messages, loading: true, ai_awaiting_approval: false, temp_flow_data: nil)
      parent = self()
        ...
        case Jason.decode(json_candidate) do
          {:ok, json_data} ->
            ...
            case Flows.create_flow(create_attrs) do
              ...
              {:error, _} ->
                assign(socket,
                  messages: socket.assigns.messages ++ [{:error, "Failed to persist flow in database"}],
                  loading: false
                )
            end
    ...
    <div class="h-full w-full flex flex-col bg-gray-50 dark:bg-slate-900 overflow-hidden">
      <div class="p-6 md:p-8 flex-shrink-0 border-b border-gray-200 dark:border-slate-800 bg-white dark:bg-slate-900">
        <div class="max-w-4xl mx-auto flex items-center gap-3">
          ...
          <div>
            <h1 class="text-2xl font-bold text-gray-900 dark:text-white"><%= gettext("Create Flow with AI") %></h1>
            <p class="text-sm text-gray-500 dark:text-slate-400 mt-1"><%= gettext("Describe what you want to automate, and I will build the flow structure for you.") %></p>
          </div>
        </div>
      </div>
      <div class="flex-1 overflow-y-auto w-full p-4 md:p-8 scroll-smooth" id="ai-chat-messages" phx-hook="ScrollToBottom">
        <div class="max-w-4xl mx-auto space-y-6 pb-20">
          <%= if Enum.empty?(@messages) do %>
            <div class="flex flex-col items-center justify-center h-full text-center text-gray-500 py-20">
              <svg class="w-16 h-16 text-indigo-200 dark:text-indigo-900/50 mb-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 10h.01M12 10h.01M16 10h.01M9 16H5a2 2 0 01-2-2V6a2 2 0 012-2h14a2 2 0 012 2v8a2 2 0 01-2 2h-5l-5 5v-5z" />
              </svg>
              <h3 class="text-lg font-medium text-gray-900 dark:text-white"><%= gettext("How can I help you today?") %></h3>
              <p class="text-sm mt-2 max-w-sm"><%= gettext("For example: 'Create a flow that listens to a webhook, logs the payload and saves it into the database.'") %></p>
            </div>
          <% end %>
            ...
            <div class={"flex w-full " <> if(role == :user, do: "justify-end", else: "justify-start")}>
              <div class={"max-w-[85%] rounded-2xl px-5 py-4 text-sm leading-relaxed prose prose-sm dark:prose-invert " <>
                  if(role == :user,
                    ...
                    else: "bg-white dark:bg-slate-800 text-gray-800 dark:text-gray-200 rounded-bl-none border border-gray-200 dark:border-slate-700 shadow-sm dark:shadow-none")}><%= if String.contains?(content, "\"action\": \"create_flow\"") or String.starts_with?(String.trim(content), "{") do %><div class="flex items-center gap-2 text-indigo-600 dark:text-indigo-400 font-medium italic animate-pulse"><.icon name="hero-arrow-path" class="w-5 h-5 animate-spin"/><%= gettext("Building flow and rendering graph...") %></div><% else %><%= markdown(content) %><% end %></div>
            </div>
          <% end %>
            ...
            <div class="flex justify-start w-full mt-4">
              <div class="ml-4 p-4 rounded-xl border-2 border-indigo-100 dark:border-indigo-900/50 bg-indigo-50/50 dark:bg-slate-800/50 flex flex-col gap-3">
                ...
                <div class="flex items-center gap-2">
                   ...
                   <span class="text-xs text-gray-400 px-2"><%= gettext("Or reply below with adjustments") %></span>
                </div>
              </div>
            </div>
          <% end %>
