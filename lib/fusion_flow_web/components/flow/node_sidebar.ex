  ...
  def node_sidebar(assigns) do
    ...
    <aside class="w-64 bg-gray-50 dark:bg-slate-800 border-r border-gray-200 dark:border-slate-700 flex flex-col z-10 shadow-lg dark:shadow-black/20">
      <div class="p-4 border-b border-gray-200 dark:border-slate-700">
        <h2 class="text-sm font-semibold text-gray-500 dark:text-slate-400 uppercase tracking-wider">Nodes</h2>
      </div>
              ...
              <%= for node <- nodes do %>
                ...
                <button
                  ...
                  class={[
                    ...
                    if(is_active,
                      do: "text-gray-700 dark:text-slate-300 bg-transparent hover:bg-gray-100 dark:hover:bg-slate-700/50 focus:outline-none focus:bg-gray-100 dark:focus:bg-slate-700/50 cursor-grab active:cursor-grabbing",
                      else: "text-gray-400 dark:text-slate-600 bg-transparent cursor-not-allowed"
                    )
                  ]}
                >
                  <div class="flex items-center gap-3">
                    <span class={["w-5 h-5 rounded flex items-center justify-center text-xs", if(is_active, do: color_class, else: "bg-gray-100 text-gray-400 dark:bg-slate-800 dark:text-slate-600")]}>
                      {node.icon}
                    </span>
                    {node.name}
                  </div>
