defmodule FusionFlowUI.FlowLive do
  ...
  def handle_event("open_dependencies_modal", _params, socket) do
    installed = FusionFlowCore.Dependencies.list_installed_mix_deps()
  ...
  def handle_event("search_dependency", %{"query" => query}, socket) do
    if String.length(query) < 2 do
      ...
      case FusionFlowCore.Dependencies.search_hex(query) do
        {:ok, results} ->
  ...
  def handle_event("install_dependency", %{"name" => name, "version" => version}, socket) do
    ...
    Task.start(fn ->
      case FusionFlowCore.Dependencies.add_dependency(name, version, "elixir", stream_to: target) do
  ...
  def handle_event("open_execution_result", %{"id" => execution_id}, socket) do
    execution = FusionFlowCore.Executions.get_execution!(execution_id)
  ...
  def render(assigns) do
    ...
    <div class="w-full h-[100vh] flex flex-col bg-white dark:bg-slate-950 overflow-hidden relative">
      ...
      <div class="flex-1 flex overflow-hidden">
        <FusionFlowUI.Components.Flow.NodeSidebar.node_sidebar
          nodes_by_category={@nodes_by_category}
      ...
      <FusionFlowUI.Components.Modals.CodeEditorModal.code_editor_modal
      ...
      />
      <FusionFlowUI.Components.Modals.ExecutionResultModal.execution_result_modal
        show_result_modal={@show_result_modal}
        execution_result={@execution_result}
        inspecting_result={@inspecting_result}
      />
      <FusionFlowUI.Components.Modals.CreateNodeModal.create_node_modal
      ...
      />
      <.live_component
        module={FusionFlowUI.Components.ChatComponent}
        id="chat-component"
        open={@chat_open}
  ...
  defp subscribe_to_execution(socket, execution) do
    if connected?(socket) do
      FusionFlowCore.Executions.subscribe_to_execution(execution)
    end
