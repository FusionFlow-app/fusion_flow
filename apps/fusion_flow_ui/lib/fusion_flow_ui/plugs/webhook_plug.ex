defmodule FusionFlowUI.Plugs.WebhookPlug do
  @moduledoc """
  ...
  def call(%Plug.Conn{path_info: ["api", "flows", flow_id, "webhook" | rest]} = conn, _opts)
    ...
    case FusionFlowCore.Webhooks.get_by_flow_and_slug(flow_id, slug) do
  ...
  defp execute_webhook(conn, flow_id) do
    flow =
      case FusionFlowCore.Flows.Cache.get(flow_id) do
        nil ->
          flow = FusionFlowCore.Flows.get_flow!(flow_id)
          FusionFlowCore.Flows.Cache.put(flow)
          flow
    ...
    with {:ok, execution} <- create_webhook_execution(flow, conn),
         {:ok, _job} <- FusionFlowCore.Executions.enqueue_execution(execution) do
      conn
      |> put_resp_content_type("application/json")
      |> send_resp(
        202,
  ...
  defp create_webhook_execution(flow, conn) do
    FusionFlowCore.Executions.create_execution(%{
      flow_id: flow.id,
      input: %{
        "trigger" => "webhook",
        "request" => %{
