defmodule FusionFlowUI.UserAuth do
  use FusionFlowUI, :verified_routes

  import Plug.Conn
  import Phoenix.Controller

  use Gettext, backend: FusionFlowUI.Gettext
  ...
  def log_out_user(conn) do
    ...
    if live_socket_id = get_session(conn, :live_socket_id) do
      FusionFlowUI.Endpoint.broadcast(live_socket_id, "disconnect", %{})
    end
  ...
  def disconnect_sessions(tokens) do
    Enum.each(tokens, fn %{token: token} ->
      FusionFlowUI.Endpoint.broadcast(user_session_topic(token), "disconnect", %{})
    end)
  end
  ...
  the `current_scope`:
      ...
      defmodule FusionFlowUI.PageLive do
        use FusionFlowUI, :live_view

        on_mount {FusionFlowUI.UserAuth, :mount_current_scope}
        ...
      end

  Or use the `live_session` of your router to invoke the on_mount callback:

      live_session :authenticated, on_mount: [{FusionFlowUI.UserAuth, :require_authenticated}] do
  ...
  def on_mount(:require_system_admin, _params, session, socket) do
    ...
    if socket.assigns.current_scope && socket.assigns.current_scope.user &&
      ...
      socket =
        ...
        |> Phoenix.LiveView.put_flash(
          :error,
          gettext("Only system administrators can access this page.")
        )
